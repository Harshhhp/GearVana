...
export async function getCarFilters() {
  try {
    ...
    // CRITICAL FIX: Log the error and return a failure object instead of throwing
    console.error("❌ ERROR fetching car filters:", error.message);
    return {
      success: false,
      error: error.message,
      // Provide empty defaults to prevent crashes in the component that expects 'data'
      data: {
        makes: [], bodyTypes: [], fuelTypes: [], transmissions: [], 
...
export async function toggleSavedCar(carId) {
  try {
    ...
    // CRITICAL FIX: Log the error and return a failure object instead of throwing
    console.error("❌ ERROR toggling saved car:", error.message);
...
export async function getCarById(carId) {
  try {
    ...
    const existingTestDrive = await db.testDriveBooking.findFirst({
      where: {
        carId,
        userId: dbUser?.id, // Use optional chaining to avoid crashing if dbUser is null
        status: { in: ["PENDING", "CONFIRMED", "COMPLETED"] },
      },
      orderBy: {
        createdAt: "desc",
